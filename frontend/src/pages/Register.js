...
function Register() {
  ...
  const subminHandler = async (e) => {
    ...
    try {
      const resp = await axios.post(
        "https://jwtapp-88nd.onrender.com/api/auth/register",
        user
      );
      //console.log(resp);
      if (resp.status == 201) {
  ...
  return (
    <Row className="d-flex justify-content-center min-vh-100 align-items-center">
      <Col lg={6}>
        <CardImg
          src="https://images.unsplash.com/photo-1702750722257-6bc38db1267a?q=80&w=1374&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          height="500"
          alt="Brand Image"
        />
      </Col>
